...
The RPM spec file provided here is for Virtualempire-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://virtualempire.in/bin/ and
...
Currently only `Source10` is used, for the example `virtualempire.conf` file.
...
in the virtualempire git repository. At present only an SVG version of the Virtualempire
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the virtualempire
...
needed when 0.12.1 is released, a proper fix is already in the Virtualempire git
master branch.
...
idea to build Virtualempire Core against a static version of that library compiled
...
The `virtualempire.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `virtualempire-server` package. At this point, most RPM based
...
The `virtualempire-server` package should have SELinux support. How to properly do
...
Virtualempire-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/virtualempire/). He has been
packaging Virtualempire for Fedora at least since 2012.
...
4. I renamed the `virtualempire` package that contains the Qt GUI to `virtualempire-core` as
that appears to be how the general population refers to it, in contrast to
`virtualempire-xt` or `virtualempire-classic`. I wanted to make sure the general population
