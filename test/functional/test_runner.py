...
`test/functional/test_framework/test_framework.py:VirtualempireTestFramework.main`.
...
BASE_SCRIPTS= [
    ...
    'rpc_net.py',
    'rpc_misc.py',
    'interface_virtualempire_cli.py',
...
def main():
    ...
    tmpdir = "%s/virtualempire_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and virtualempired
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_virtualempired = config["components"].getboolean("ENABLE_VIRTUALEMPIRED")
    if not (enable_wallet and enable_cli and enable_virtualempired):
        print("No functional tests to run. Wallet, utils, and virtualempired must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if virtualempired is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "virtualempired"]) is not None:
            print("%sWARNING!%s There is already a virtualempired process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "VIRTUALEMPIRED" not in os.environ:
        os.environ["VIRTUALEMPIRED"] = build_dir + '/src/virtualempired' + exeext
        os.environ["VIRTUALEMPIRECLI"] = build_dir + '/src/virtualempire-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `virtualempire-cli help` (`rpc_interface.txt`).
