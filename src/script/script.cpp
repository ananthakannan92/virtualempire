...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** VIRTUALEMPIRE START */
    case OP_VTE_ASSET              : return "OP_VTE_ASSET";
    /** VIRTUALEMPIRE END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_VTE_ASSET) { // OP_VTE_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == VTE_N) { // Check to see if VTE starts at 27 ( this->size() < 105)
                if ((*this)[28] == VTE_E)
                    if ((*this)[29] == VTE_X)
                        index = 30;
            } else {
                if ((*this)[28] == VTE_N) // Check to see if VTE starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == VTE_E)
                        if ((*this)[30] == VTE_X)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == VTE_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == VTE_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == VTE_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == VTE_N) {
...
{
    // 1 OP_VTE_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_VTE_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_VTE_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_VTE_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_VTE_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
