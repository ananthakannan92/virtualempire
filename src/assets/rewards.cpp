...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for VTE
    ...
    if (p_rewardSnapshot.strDistributionAsset != "VTE") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is VTE with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using VTE differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "VTE") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Virtualempire address (already validated during ownership snapshot creation)
