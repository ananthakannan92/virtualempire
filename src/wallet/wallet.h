...
#ifndef VIRTUALEMPIRE_WALLET_WALLET_H
#define VIRTUALEMPIRE_WALLET_WALLET_H
...
public:
     ...
     * on this virtualempire node, and set to 0 for transactions that were created
...
public:
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins,
                            bool fGetVTE = true, bool fOnlyAssets = false,
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Assets and VTE
     */
...
#endif // VIRTUALEMPIRE_WALLET_WALLET_H
