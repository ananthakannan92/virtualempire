...
#include "virtualempireaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a virtualempire address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new VirtualempireAddressEntryValidator(parent));
    widget->setCheckValidator(new VirtualempireAddressCheckValidator(parent));
}
...
bool parseVirtualempireURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no virtualempire: URI
    if(!uri.isValid() || uri.scheme() != QString("virtualempire"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!VirtualempireUnits::parse(VirtualempireUnits::VTE, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseVirtualempireURI(QString uri, SendCoinsRecipient *out)
{
    // Convert virtualempire:// to virtualempire:
    //
    //    Cannot handle this later, because virtualempire:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("virtualempire://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "virtualempire:");
    }
    QUrl uriInstance(uri);
    return parseVirtualempireURI(uriInstance, out);
}

QString formatVirtualempireURI(const SendCoinsRecipient &info)
{
    QString ret = QString("virtualempire:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(VirtualempireUnits::format(VirtualempireUnits::VTE, info.amount, false, VirtualempireUnits::separatorNever));
        paramCount++;
    }
...
bool openVirtualempireConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", VIRTUALEMPIRE_CONF_FILENAME));

    /* Open virtualempire.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Virtualempire.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Virtualempire (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Virtualempire (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Virtualempire*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "virtualempire.desktop";
    return GetAutostartDir() / strprintf("virtualempire-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a virtualempire.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Virtualempire\n";
        else
            optionFile << strprintf("Name=Virtualempire (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the virtualempire app
...
bool GetStartOnSystemStartup()
{
    CFURLRef virtualempireAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (virtualempireAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, virtualempireAppUrl);

    CFRelease(virtualempireAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add virtualempire app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, virtualempireAppUrl, nullptr, nullptr);
    }
