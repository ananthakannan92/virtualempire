    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return VirtualempireUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, VirtualempireUnits::separatorNever);
            else
                return VirtualempireUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+VirtualempireUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
