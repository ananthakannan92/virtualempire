...
#include "virtualempiregui.h"

#include "virtualempireunits.h"
...
const std::string VirtualempireGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString VirtualempireGUI::DEFAULT_WALLET = "~Default";

VirtualempireGUI::VirtualempireGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
VirtualempireGUI::~VirtualempireGUI()
{
...
void VirtualempireGUI::loadFonts()
{
...
void VirtualempireGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a virtualempire address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and virtualempire: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to VTE addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your virtualempire addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified virtualempire addresses"));
    ...
    openAction->setStatusTip(tr("Open a Virtualempire: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible virtualempire command-line options").arg(tr(PACKAGE_NAME)));
...
void VirtualempireGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void VirtualempireGUI::createToolBars()
{
    if(walletFrame)
    {
        /** VIRTUALEMPIRE START */
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* virtualempireLabelLayout = new QVBoxLayout(toolbarWidget);
        virtualempireLabelLayout->addWidget(label);
        virtualempireLabelLayout->addWidget(toolbar);
        virtualempireLabelLayout->setDirection(QBoxLayout::TopToBottom);
        virtualempireLabelLayout->addStretch(1);
        ...
        // Get the latest Virtualempire release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void VirtualempireGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool VirtualempireGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool VirtualempireGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void VirtualempireGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void VirtualempireGUI::setWalletActionsEnabled(bool enabled)
{
...
void VirtualempireGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void VirtualempireGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void VirtualempireGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void VirtualempireGUI::optionsClicked()
{
...
void VirtualempireGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void VirtualempireGUI::showDebugWindow()
{
...
void VirtualempireGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void VirtualempireGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void VirtualempireGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void VirtualempireGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void VirtualempireGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void VirtualempireGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void VirtualempireGUI::openClicked()
{
...
void VirtualempireGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void VirtualempireGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void VirtualempireGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void VirtualempireGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void VirtualempireGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void VirtualempireGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** VIRTUALEMPIRE START */
void VirtualempireGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void VirtualempireGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void VirtualempireGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void VirtualempireGUI::gotoRestrictedAssetsPage()
{
...
void VirtualempireGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to virtualempire network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void VirtualempireGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void VirtualempireGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void VirtualempireGUI::updateHeadersSyncProgressLabel()
{
...
void VirtualempireGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void VirtualempireGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Virtualempire"); // default title
...
void VirtualempireGUI::changeEvent(QEvent *e)
{
...
void VirtualempireGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void VirtualempireGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void VirtualempireGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "VTE")
        msg += tr("Amount: %1\n").arg(VirtualempireUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(VirtualempireUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void VirtualempireGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void VirtualempireGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool VirtualempireGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool VirtualempireGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void VirtualempireGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void VirtualempireGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void VirtualempireGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void VirtualempireGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void VirtualempireGUI::detectShutdown()
{
...
void VirtualempireGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void VirtualempireGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void VirtualempireGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(VirtualempireGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(VirtualempireGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void VirtualempireGUI::subscribeToCoreSignals()
{
...
void VirtualempireGUI::unsubscribeFromCoreSignals()
{
...
void VirtualempireGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void VirtualempireGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<VirtualempireUnits::Unit> units = VirtualempireUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const VirtualempireUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(VirtualempireUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(VirtualempireUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (VirtualempireUnits::Unit u : VirtualempireUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(VirtualempireUnits::name(u)), this);
...
{
    setText(VirtualempireUnits::name(newUnits));
}
...
void VirtualempireGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=VTEBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void VirtualempireGUI::mnemonic()
{
