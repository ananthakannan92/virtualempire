 ...
 * between virtualempired, and virtualempire-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("virtualempire-shutoff");
    ...
    FlushWallets();
#endif
    GenerateVirtualempires(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), VIRTUALEMPIRE_CONF_FILENAME));
    if (mode == HMM_VIRTUALEMPIRED)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), VIRTUALEMPIRE_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/JustAResearcher/Virtualempire>";
    const std::string URL_WEBSITE = "<https://virtualempire.lol>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("virtualempire-loadblk");
...
/** Sanity checks
 *  Ensure that Virtualempire is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Virtualempire version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Virtualempire process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", VIRTUALEMPIRE_CONF_FILENAME)).string());
    ...
    GenerateVirtualempires(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
