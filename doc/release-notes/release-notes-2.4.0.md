Virtualempire Core version *2.4.0* is now available!!
==============
...
installer (on Windows) or just copy over `/Applications/Virtualempire-Qt` (on Mac)
or `virtualempired`/`virtualempire-qt` (on Linux).
...
Virtualempire Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows 10(x64) and later. 32-bit versions of Windows,
and Windows XP are not supported.

Virtualempire Core should also work on most other Unix-like systems but is not
frequently tested on them.

Virtualempire Core has been tested with macOS 10.14 Mojave, but it is recommended that *developers*
...
Virtualempire Core has not been tested with macOS Catalina(beta). Please use at your own risk.
...
Changelog available here: <https://github.com/JustAResearcher/Virtualempire/commits/release_2.4.0>
...
- Most importantly - The Virtualempire Community!
